...
void host_comm_process_request(comm_cmd_t cmd, uint8_t *pData, uint32_t nDataLen)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_EMC_DEBUG_REG:
        {
            ...
            if(!res)
            {
                LOG_ERROR("%s: emc230x_debug_registers failed.", __FUNCTION__);
        ...
        {
            LOG_DEBUG("CMD_JUMP_TO_BOOTLOADER");
            jump_to_bootloader();
            break;
        }
        ...
        default:
        {
            LOG_ERROR("%s: Unsupported or invalid command", __FUNCTION__);
        }
    }
...
void host_comm_parse_package(uint8_t *pPackageStart, uint32_t nLen)
{
    ...
    {
        host_comm_process_request(static_cast<comm_cmd_t>(pPackage[0]), &pPackage[1], nPackageLen-1);
    }
}
...
void host_comm_tick(void)
{
    ...
    if (bRxComplete)
    {
        LOG_INFO("%s", pRxBuffer);

        if(nRxBufferLen < COMM_MIN_MESSAGE_LEN)
        {
            LOG_ERROR("%s: Invalid message lenght (%ld<%d)", __FUNCTION__, nRxBufferLen, COMM_MIN_MESSAGE_LEN);
            host_comm_rearm();
            return;
        }
        ...
        for(uint32_t i=0; i<nRxBufferLen; i++)
        {
            ...
            {
                LOG_DEBUG("%s: Start character found (Len=%ld)", __FUNCTION__, nRxBufferLen-i-1);
        ...
        if(!bValidMsg)
        {
            LOG_ERROR("%s: No start character found. Ignoring package.", __FUNCTION__);
        }
...
static uint8_t buffer_to_uint8(uint8_t *pBuffer)
{
    if(pBuffer==NULL)
    {
        LOG_ERROR("%s Invalid pointer!", __FUNCTION__);
        return 0;
    }
...
static uint8_t ascii_to_hex(uint8_t ascii)
{
    ...
    {
        LOG_ERROR("%s:%d Invalid character! (%c - 0x%02X)", __FUNCTION__, __LINE__, ascii, ascii);
        return 0;
    }
}
...
static bool ascii_array_to_hex_array(uint8_t *pASCII, uint32_t nLen, uint8_t *pTarget, uint32_t *nHexLen)
{
    ...
    if(nLen<=0)
    {
        LOG_ERROR("%s:%d Invalid buffer length (%ld)", __FUNCTION__, __LINE__, nLen);
        return false;
    }
    ...
    for (uint32_t i=0; i<nLen; i+=2)
    {
        ...
        LOG_DEBUG("A=%c%c H:0x%02X", pASCII[i], pASCII[i+1], raw);
