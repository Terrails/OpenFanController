...
#define FAN_TICK_PERIOD 1000000
...
{
    LOG_INFO("---Fan Controller---");
    LOG_INFO("Fan Controllers: %d", NUM_CONTROLLERS);
    LOG_INFO("Fans per controller: %d", NUM_FAN_PER_CONTROLLER);
    LOG_INFO("Total fans: %d", NUM_TOTAL_FAN);
...
{
    int32_t elapsed_time = (int32_t) (time_us_32() - nFanTick);
    ...
    // if (time_us_32() >= nFanTick)
    {
        ...
        LOG_DEBUG("1:%4d 2:%4d 3:%4d 4:%4d 5:%4d 6:%4d 7:%4d 8:%4d 9:%4d 10:%4d",
                     fan_rpm[0],
                     fan_rpm[1],
                     fan_rpm[2],
                     fan_rpm[3],
        ...
        // LOG_INFO("Fan Stall: 0x%02X", reg);

        // emc230x_read_register(0x26, &reg);
        // LOG_INFO("Fan SPIN: 0x%02X", reg);

        // emc230x_read_register(0x27, &reg);
        // LOG_INFO("Driver Fail: 0x%02X", reg);

        // emc230x_read_register(0x32, &reg);
        // LOG_INFO("Fan 1: 0x%02X", reg);
...
void fan_control_read_fan_rpm(uint8_t fan, uint16_t *pRPM)
{
    ...
    if(pRPM == NULL)
    {
        LOG_ERROR("%s: Null pointer. Aborting!", __FUNCTION__);
        return;
    }
    if(fan >= NUM_TOTAL_FAN)
    {
        LOG_ERROR("%s: Requested fan index (%d) is out of bounds", __FUNCTION__, fan);
        *pRPM = 0;
        return;
    }
    ...
    *pRPM = fan_rpm[fan];
    ...
    emc230x_read_fan_rpm(fan_channel_map[fan], pRPM);
    LOG_DEBUG("%s: Fan:%d RPM:%d", __FUNCTION__, fan, *pRPM);
    #endif
}
...
void fan_control_read_all_rpm(uint16_t *pRPM, uint8_t nLen)
{
    ...
    {
        LOG_ERROR("%s: Requested number of fans (%d) is greater than total number of fans (%d). Clipping.", __FUNCTION__, nLen, NUM_TOTAL_FAN);
        nLen = NUM_TOTAL_FAN;
    }
...
void fan_control_set_fan_rpm(uint8_t fan, uint16_t nRPM)
{
    ...
    LOG_DEBUG("%s: Fan:%d RPM:%d", __FUNCTION__, fan, nRPM);
    switch_fan_controller_to(fan_controller_map[fan]);
...
void fan_control_set_fan_pwm(uint8_t fan, uint8_t pwm)
{
    if(pwm>255)
    {
        LOG_ERROR("%s: Requested PWM[%d] > 255. Clipping to 255", __FUNCTION__, pwm);
        pwm = 255;
    }
    if(fan>=NUM_TOTAL_FAN)
    {
    ...
    LOG_DEBUG("%s: Fan:%d PWM:%d", __FUNCTION__, fan, pwm);
    switch_fan_controller_to(fan_controller_map[fan]);
...
void fan_control_set_all_pwm(uint8_t pwm)
{
    ...
    LOG_DEBUG("%s: Setting ALL fans to PWM:%d", __FUNCTION__, pwm);
...
static void switch_fan_controller_to(uint8_t controller)
{
    ...
    {
        LOG_ERROR("%s: Unknown or unsupported controller!", __FUNCTION__);
    }
}
...
static uint8_t map_channel_to_controller(uint8_t channel)
{
    switch(channel)
    {
        ...
        default:
            LOG_ERROR("Unknown FAN location.");
            emc230x_set_address(EMC_CTRL_1_ADDRESS);
            return 0;
            break;
