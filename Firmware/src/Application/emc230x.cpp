...
void emc230x_config_fans(void)
{
    ...
    {
        // Enable closed loop algorithm, 500 RPM minimum measured, 2 poles, 200 ms update interval
        // emc230x_write_register(EMC2305_REG_FAN_CONFIG(i), 0b10001001);

        // Enable ramp rate control, glitch filter, none derivative options, 50 RPM tolerance
        emc230x_write_register(EMC2305_REG_FAN_CONFIG2(i), 0b01100010);
    }
}
...
void emc230x_read_fan_rpm(uint8_t channel, uint16_t *pRPM)
{
    ...
    {
        LOG_ERROR("Invalid fan channel selection! (%d>=%d)", channel, EMC230X_DEVICE_MAX_CHANNELS);
        return;
    }

    if(pRPM == NULL)
    {
        LOG_ERROR("Invalid RPM pointer!");
        return;
    }
...
void emc230x_set_target_rpm(uint8_t channel, uint16_t nRPM)
{
    ...
    if(nRPM > 160000)
    {
        LOG_ERROR("Target RPM is higher than IC can support. Clipping.");
        nRPM = 16000;
    }
    ...
    LOG_INFO("Setting RPM to %d (H:0x%02X L:0x%02X)", nRPM, high, low);
...
void emc230x_set_pwm(uint8_t channel, uint8_t nPWM)
{
    ...
    if(res)
    {
        LOG_DEBUG("%s: Channel %d set to %d", __FUNCTION__, channel, nPWM);
    }
    else
    {
        LOG_ERROR("%s: emc230x_send failed!", __FUNCTION__);
    }
}
...
bool emc230x_debug_registers(uint8_t startRegister, uint8_t *pData, uint16_t nReadCount)
{
    ...
    LOG_INFO("Reading %d registers starting from 0x%02X", nReadCount, startRegister);
...
static bool emc230x_send(uint8_t *pData, uint16_t nLen)
{
    ...
    if(status <= 0)
    {
        LOG_ERROR("%s: Error 0x%02X", __FUNCTION__, status);
        return false;
    }
    return true;
}
...
static bool emc230x_send_then_read(uint8_t *pTXData, uint16_t nTXLen, uint8_t *pRXData, uint16_t nRXLen)
{
    ...
    if(status <= 0)
    {
        LOG_ERROR("%s: write error 0x%02X", __FUNCTION__, status);
        return false;
    }
    ...
    if(status <= 0)
    {
        LOG_ERROR("%s: read error 0x%02X", __FUNCTION__, status);
        return false;
    }
    return true;
}
